  ...
  render() {
    ...
    list.forEach((val, index) => {
      ...
      if (status.COMPLETE === 1)
        mode = 5;
      else if (status.SWAP.findIndex(val1 => index === val1) !== -1)
        mode = 1;
      else if (status.COMPARE.findIndex(val1 => index === val1) !== -1)
        mode = 3;
      else if (status.PIVOT === index)
        mode = 2;
      else if (status.CURSOR === index)
        mode = 4;
      else if (status.STORE.i && status.STORE.i === index)
        mode = 1;
      else if (status.BOUNDARY.findIndex(val1 => val1 === index) !== -1)
        mode = 4;
      data.push(<Rectangle key = { index } mode = { mode } pos = {
        index
      } height = {val.height} />);
    });

    return (<svg viewBox = " 0 0 1070 900" style = {
      { background: "black" }
    } onClick = {() => { this.sortRender(); }}>{data}</ svg>);
  }

  async sortRender() {
    let action = selectionSort(this.props.data.collection.list);
...
const mapDispatchToProps = dispatch => ({
  swap(i, j) { dispatch(swapItem(i, j)); },
  compare(i, j) { dispatch(compareItem(i, j)); },
  store(i, val) { dispatch(storeItem(i, val)); },
  pivot(i) { dispatch(setPivot(i)); },
  boundary(i, j) { dispatch(setBoundary(i, j)); },
  complete(val) { dispatch(completeList(val)); }
});
